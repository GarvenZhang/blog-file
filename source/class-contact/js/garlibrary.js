(function (window) {
  ...
  gar.subCookieUtil = {
    ...
    setAll: function (name, subCookies, expires, path, domain, secure) {
      ...
      for (subName in subCookies) {
        if (subName.length > 0 && subCookies.hasOwnProperty(subName)) { subCookieParts.push(encodeURIComponent(subName) + '=' + encodeURIComponent(subCookies[subName])) }
      }
  ...
  gar.hasPlugin = function (name) {
    ...
    for (var np = navigator.plugins, l = np.length; l--;) {
      if (np[l].name.toLowerCase().indexOf(name) > -1) { return true }
    }
    return false
  }
