/**
 * Created by Garven Zhang on 2017/8/9.
 * @overView javascript code library
 * @copyright better to read
 * @author Garven Zhang jf00258jf@hotmail.com
 * @version 1.0.1
 */
(function (window, undefined) {
  var gar = (function () {
    var gar = function (selector, context) {
      return new gar.fn.init(selector, context, rootgar)
    }

    gar.fn = gar.prototype = {
      constructor: gar,
      init: function (selector, context, rootgar) {

      }
    }
    gar.fn.init.prototype = gar.fn

    gar.extend = gar.fn.extend = function () {
  ...
  gar.each = function (obj, callback) {
    ...
    var val, i = -1,
      ...
      l = obj.length
    if (isArr) { for (; o = obj[++i];) val = callback.call(o, i, o, l) } else { for (o in obj) val = callback.call(obj[o], ++i, obj[o], l, o) }
    return obj
  }
    ...
    if (!eleOrObj.nodeType) {
      for (var className in eleOrObj) {
        if (!new RegExp('\\b' + className + '\\b', 'gi')
                        .test(eleOrObj[className].className)) { eleOrObj[className].className = eleOrObj[className].className.concat(' ' + className) }
      }
    } else if (!new RegExp('\\b' + classname + '\\b', 'gi')
                .test(eleOrObj.className)) { eleOrObj.className = eleOrObj.className.concat(' ' + classname) }
  }
  ...
  gar.getSelectionTxt = function (txtbox) {
    if (typeof (txtbox.selectionStart) === 'number') {
      this.getSelectionTxt = function (txtbox) {
  ...
  gar.blobSlice = function (blob, startByte, length) {
    if (blob.slice) {
      this.blobSlice = function (blob, startByte, length) {
  ...
  gar.createObjURL = function (blob) {
    if (window.URL) {
      this.createObjURL = function (blob) {
  ...
  gar.revokeObjURL = function (url) {
    if (window.URL) {
      this.revokeObjURL = function (url) {
  ...
  gar.Set.prototype = {
    ...
    union: function (set) {
      ...
      for (var sD = set.dataStore, sL = sD.length; sL--;) {
        if (!tmpSet.contains(sD[sL])) { tmpSet.dataStore.push(sD[sL]) }
      }
      return tmpSet
    },
    intersect: function (set) {
      ...
      for (var d = this.dataStore, l = d.length; l--;) {
        if (set.contains(d[l])) { tmpSet.dataStore.push(d[l]) }
      }
      return tmpSet
    },
    subset: function (set) {
