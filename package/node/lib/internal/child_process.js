...
ChildProcess.prototype.spawn = function (options) {
  ...
  if (ipc !== undefined) {
    ...
    if (options.envPairs === undefined) { options.envPairs = [] } else if (!Array.isArray(options.envPairs)) {
  ...
  if (Array.isArray(options.args)) { this.spawnargs = options.args } else if (options.args === undefined) { this.spawnargs = [] } else { throw new ERR_INVALID_ARG_TYPE('options.args', 'Array', options.args) }
  ...
  for (i = 0; i < stdio.length; i++) { this.stdio.push(stdio[i].socket === undefined ? null : stdio[i].socket) }
