...
function onlookupall (err, addresses) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    addresses[i] = {
      address,
      family: family || isIPv4(addresses[i]) ? 4 : 6
    }
  }
...
function createLookupPromise (family, hostname, all, hints, verbatim) {
  return new Promise((resolve, reject) => {
    if (!hostname) {
      if (all) { resolve([]) } else { resolve({ address: null, family: family === 6 ? 6 : 4 }) }
    ...
    if (matchedFamily !== 0) {
      ...
      if (all) { resolve([result]) } else { resolve(result) }
...
function lookup (hostname, options) {
  ...
  if (family !== 0 && family !== 4 && family !== 6) { throw new ERR_INVALID_OPT_VALUE('family', family) }
...
function createLookupServicePromise (host, port) {
  return new Promise((resolve, reject) => {
    ...
    if (err) { reject(dnsException(err, 'getnameinfo', host)) }
  })
}

function lookupService (host, port) {
  if (arguments.length !== 2) { throw new ERR_MISSING_ARGS('host', 'port') }

  if (isIP(host) === 0) { throw new ERR_INVALID_OPT_VALUE('host', host) }

  if (!isLegalPort(port)) { throw new ERR_SOCKET_BAD_PORT(port) }
...
function onresolve (err, result, ttls) {
  ...
  if (ttls && this.ttl) { result = result.map((address, index) => ({ address, ttl: ttls[index] })) }
...
Resolver.prototype.resolve = function resolve (hostname, rrtype) {
  ...
  if (typeof rrtype === 'string') {
    ...
    if (typeof resolver !== 'function') { throw new ERR_INVALID_OPT_VALUE('rrtype', rrtype) }
