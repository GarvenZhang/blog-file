...
class Loader {
  ...
  async resolve (specifier, parentURL) {
    ...
    if (!isMain && typeof parentURL !== 'string') { throw new ERR_INVALID_ARG_TYPE('parentURL', 'string', parentURL) }
    ...
    if (format === 'builtin') { return { url: `node:${url}`, format } }
  ...
  hook ({ resolve, dynamicInstantiate }) {
    ...
    if (resolve !== undefined) { this._resolve = FunctionBind(resolve, null) }
    if (dynamicInstantiate !== undefined) { this._dynamicInstantiate = FunctionBind(dynamicInstantiate, null) }
  }
  ...
  async getModuleJob (specifier, parentURL) {
    ...
    let loaderInstance
    if (format === 'dynamic') {
      if (typeof this._dynamicInstantiate !== 'function') { throw new ERR_MISSING_DYNAMIC_INSTANTIATE_HOOK() }
