...
function ReadStream (path, options) {
  ...
  if (this.start !== undefined) {
    if (!Number.isSafeInteger(this.start)) {
      if (typeof this.start !== 'number') { throw new ERR_INVALID_ARG_TYPE('start', 'number', this.start) }
      if (!Number.isInteger(this.start)) { throw new ERR_OUT_OF_RANGE('start', 'an integer', this.start) }
  ...
  if (this.end === undefined) {
    ...
    if (!Number.isSafeInteger(this.end)) {
      if (typeof this.end !== 'number') { throw new ERR_INVALID_ARG_TYPE('end', 'number', this.end) }
      if (!Number.isInteger(this.end)) { throw new ERR_OUT_OF_RANGE('end', 'an integer', this.end) }
...
ReadStream.prototype._read = function (n) {
  ...
  if (this.pos !== undefined) { toRead = Math.min(this.end - this.pos + 1, toRead) } else { toRead = Math.min(this.end - this.bytesRead + 1, toRead) }
