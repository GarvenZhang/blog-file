...
function validateFileHandle (handle) {
  if (!(handle instanceof FileHandle)) { throw new ERR_INVALID_ARG_TYPE('filehandle', 'FileHandle', handle) }
}
...
async function readFileHandle (filehandle, options) {
  ...
  if (size > kMaxLength) { throw new ERR_FS_FILE_TOO_LARGE(size) }
  ...
  do {
    ...
    if (bytesRead > 0) { chunks.push(buffer.slice(0, bytesRead)) }
  } while (!endOfFile)
...
async function write (handle, buffer, offset, length, position) {
  ...
  if (buffer.length === 0) { return { bytesWritten: 0, buffer } }
...
async function writeFile (path, data, options) {
  ...
  if (path instanceof FileHandle) { return writeFileHandle(path, data, options) }
