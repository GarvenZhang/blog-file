...
function onStreamClose (code) {
  ...
  if (!stream.readable || code !== NGHTTP2_NO_ERROR) {
    ...
    if (stream[kSession][kType] === NGHTTP2_SESSION_SERVER &&
        ...
        stream.readableFlowing === null) { stream.resume() } else { stream.read(0) }
  }
}
...
class Http2Session extends EventEmitter {
  ...
  setNextStreamID (id) {
    ...
    if (typeof id !== 'number') { throw new ERR_INVALID_ARG_TYPE('id', 'number', id) }
  ...
  ping (payload, callback) {
    ...
    if (typeof callback !== 'function') { throw new ERR_INVALID_CALLBACK() }
  ...
  settings (settings, callback) {
    ...
    if (callback && typeof callback !== 'function') { throw new ERR_INVALID_CALLBACK() }
  ...
  destroy (error = NGHTTP2_NO_ERROR, code) {
    ...
    if (error) {
      cancel.cause = error
      if (typeof error.message === 'string') { cancel.message += ` (caused by: ${error.message})` }
    }
    ...
    if (handle !== undefined) { handle.destroy(code, socket.destroyed) }
...
class ServerHttp2Session extends Http2Session {
  ...
  altsvc (alt, originOrStream) {
    ...
    if (typeof alt !== 'string') { throw new ERR_INVALID_ARG_TYPE('alt', 'string', alt) }
    if (!kQuotedString.test(alt)) { throw new ERR_INVALID_CHAR('alt') }

    // Max length permitted for ALTSVC
    if ((alt.length + (origin !== undefined ? origin.length : 0)) > 16382) { throw new ERR_HTTP2_ALTSVC_LENGTH() }
...
class ClientHttp2Session extends Http2Session {
  ...
  request (headers, options) {
    ...
    if (!connect) {
      ...
      if (headers[HTTP2_HEADER_AUTHORITY] === undefined) { throw new ERR_HTTP2_CONNECT_AUTHORITY() }
      if (headers[HTTP2_HEADER_SCHEME] !== undefined) { throw new ERR_HTTP2_CONNECT_SCHEME() }
      if (headers[HTTP2_HEADER_PATH] !== undefined) { throw new ERR_HTTP2_CONNECT_PATH() }
    }
...
function closeStream (stream, code, rstStreamStatus = kSubmitRstStream) {
  ...
  if (rstStreamStatus !== kNoRstStream) {
    ...
    if (!ending || finished || code !== NGHTTP2_NO_ERROR ||
        rstStreamStatus === kForceRstStream) { finishFn() } else { stream.once('finish', finishFn) }
  }
}
...
class Http2Stream extends Duplex {
  ...
  [kWriteGeneric] (writev, data, encoding, cb) {
    ...
    if (writev) { writevGeneric(this, req, data, cb) } else { writeGeneric(this, req, data, encoding, cb) }
  ...
  close (code = NGHTTP2_NO_ERROR, callback) {
    if (typeof code !== 'number') { throw new ERR_INVALID_ARG_TYPE('code', 'number', code) }
    if (code < 0 || code > kMaxInt) { throw new ERR_OUT_OF_RANGE('code', `>= 0 && <= ${kMaxInt}`, code) }
    if (callback !== undefined && typeof callback !== 'function') { throw new ERR_INVALID_CALLBACK() }
...
function processHeaders (headers) {
  ...
  if (statusCode < 200 || statusCode > 599) { throw new ERR_HTTP2_STATUS_INVALID(headers[HTTP2_HEADER_STATUS]) }
...
function onFileUnpipe () {
  ...
  if (stream.ownsFd) { this.source.close().catch(onFileCloseError.bind(stream)) } else { this.source.releaseFD() }
}
...
class ServerHttp2Stream extends Http2Stream {
  ...
  respondWithFD (fd, headers, options) {
    ...
    if (options.offset !== undefined && typeof options.offset !== 'number') { throw new ERR_INVALID_OPT_VALUE('offset', options.offset) }

    if (options.length !== undefined && typeof options.length !== 'number') { throw new ERR_INVALID_OPT_VALUE('length', options.length) }
    ...
    if (typeof fd !== 'number') { throw new ERR_INVALID_ARG_TYPE('fd', 'number', fd) }
