...
function strictDeepEqual (val1, val2, memos) {
  ...
  if (isDate(val1)) {
    ...
    if (val1Value !== val1) {
      if (!innerDeepEqual(val1Value, val2.valueOf(), true)) { return false }
...
function keyCheck (val1, val2, strict, memos, lengthA, lengthB) {
  ...
  if (strict) {
    ...
    if (lengthA !== undefined) {
      if (aKeys.length !== lengthA || bKeys.length !== lengthB) { return false }
      if (symbolKeysA.length === 0) { return true }
      aKeys = []
      bKeys = []
    }
    if (symbolKeysA.length !== 0) {
...
function innerDeepEqual (val1, val2, strict, memos) {
  ...
  if (strict === true) { return strictDeepEqual(val1, val2, memos) }
