...
class Module {
  constructor (src, options = {}) {
    ...
    if (typeof src !== 'string') { throw new ERR_INVALID_ARG_TYPE('src', 'string', src) }
    if (typeof options !== 'object' || options === null) { throw new ERR_INVALID_ARG_TYPE('options', 'Object', options) }
  ...
  get error () {
    const wrap = wrapMap.get(this)
    if (wrap.getStatus() !== kErrored) { throw new ERR_VM_MODULE_STATUS('must be errored') }
    return wrap.getError()
  }

  async link (linker) {
    if (typeof linker !== 'function') { throw new ERR_INVALID_ARG_TYPE('linker', 'function', linker) }
    if (linkingStatusMap.get(this) !== 'unlinked') { throw new ERR_VM_MODULE_ALREADY_LINKED() }
    const wrap = wrapMap.get(this)
    if (wrap.getStatus() !== kUninstantiated) { throw new ERR_VM_MODULE_STATUS('must be uninstantiated') }
    ...
    const promises = wrap.link(async (specifier) => {
      ...
      if (m.context !== this.context) { throw new ERR_VM_MODULE_DIFFERENT_CONTEXT() }
      const childLinkingStatus = linkingStatusMap.get(m)
      if (childLinkingStatus === 'errored') { throw new ERR_VM_MODULE_LINKING_ERRORED() }
      if (childLinkingStatus === 'unlinked') { await m.link(linker) }
      return wrapMap.get(m)
    })

    try {
      if (promises !== undefined) { await SafePromise.all(promises) }
  ...
  instantiate () {
    ...
    if (status === kInstantiating || status === kEvaluating) { throw new ERR_VM_MODULE_STATUS('must not be instantiating or evaluating') }
    if (linkingStatusMap.get(this) !== 'linked') { throw new ERR_VM_MODULE_NOT_LINKED() }
    wrap.instantiate()
  }
  ...
  [customInspectSymbol] (depth, options) {
    ...
    if (typeof depth === 'number' && depth < 0) { return options.stylize(`[${ctor.name}]`, 'special') }
