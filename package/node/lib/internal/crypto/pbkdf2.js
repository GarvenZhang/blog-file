...
function check (password, salt, iterations, keylen, digest, callback) {
  if (typeof digest !== 'string') {
    if (digest !== null) { throw new ERR_INVALID_ARG_TYPE('digest', ['string', 'null'], digest) }
    digest = 'sha1'
  }
...
function handleError (keybuf, password, salt, iterations, digest, wrap) {
  ...
  if (rc === -1) { throw new ERR_CRYPTO_INVALID_DIGEST(digest) }

  if (rc === false) { throw new ERR_CRYPTO_PBKDF2_ERROR() }
}
