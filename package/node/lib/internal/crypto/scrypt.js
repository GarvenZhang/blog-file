...
function handleError (keybuf, password, salt, N, r, p, maxmem, wrap) {
  ...
  if (ex === null) { throw new ERR_CRYPTO_SCRYPT_INVALID_PARAMETER() }  // Bad N, r, p, or maxmem.

  throw ex  // Scrypt operation failed, exception object contains details.
}

function check (password, salt, keylen, options, callback) {
  if (_scrypt === undefined) { throw new ERR_CRYPTO_SCRYPT_NOT_SUPPORTED() }
  ...
  if (options && options !== defaults) {
    if (options.hasOwnProperty('N')) { N = validateInt32(options.N, 'N', 0, INT_MAX) }
    if (options.hasOwnProperty('r')) { r = validateInt32(options.r, 'r', 0, INT_MAX) }
    if (options.hasOwnProperty('p')) { p = validateInt32(options.p, 'p', 0, INT_MAX) }
    if (options.hasOwnProperty('maxmem')) { maxmem = validateInt32(options.maxmem, 'maxmem', 0, INT_MAX) }
    if (N === 0) N = defaults.N
    if (r === 0) r = defaults.r
    if (p === 0) p = defaults.p
