...
function Hash (algorithm, options) {
  ...
  if (typeof algorithm !== 'string') { throw new ERR_INVALID_ARG_TYPE('algorithm', 'string', algorithm) }
...
Hash.prototype.update = function update (data, encoding) {
  ...
  if (!this._handle.update(data, encoding || getDefaultEncoding())) { throw new ERR_CRYPTO_HASH_UPDATE_FAILED() }
  return this
}
...
Hash.prototype.digest = function digest (outputEncoding) {
  ...
  if (normalizeEncoding(outputEncoding) === 'utf16le') { throw new ERR_CRYPTO_HASH_DIGEST_NO_UTF16() }
...
function Hmac (hmac, key, options) {
  ...
  if (typeof hmac !== 'string') { throw new ERR_INVALID_ARG_TYPE('hmac', 'string', hmac) }
