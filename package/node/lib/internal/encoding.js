...
function validateEncoder (obj) {
  if (obj == null || obj[kEncoder] !== true) { throw new ERR_INVALID_THIS('TextEncoder') }
}
...
function validateArgument (prop, expected, propName, expectedName) {
  if (typeof prop !== expected) { throw new ERR_INVALID_ARG_TYPE(propName, expectedName, prop) }
}
...
function makeTextDecoderJS () {
  ...
  function lazyStringDecoder () {
    if (StringDecoder === undefined) { ({ StringDecoder } = require('string_decoder')) }
  ...
  class TextDecoder {
    constructor (encoding = 'utf-8', options = {}) {
      ...
      const enc = getEncodingFromLabel(encoding)
      if (enc === undefined || !hasConverter(enc)) { throw new ERR_ENCODING_NOT_SUPPORTED(encoding) }
